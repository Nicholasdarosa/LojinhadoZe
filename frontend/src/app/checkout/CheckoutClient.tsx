...
type CartItem = {
  ...
  name: string;
  quantity: number;
  price: number; // unitário em REAIS
  total: number; // total da linha em REAIS
  imageUrl?: string;
};
...
/**
 * Converte número vindo da API em REAIS.
 * Aqui estamos assumindo que o /api/cart devolve valores em CENTAVOS
 * (ex.: 500 => R$ 5,00).
 */
 ...
 * ASSUME que price/total/subtotal vêm em CENTAVOS.
 */
function mapRawCart(raw: any): CartState {
  ...
  const items: CartItem[] = rawItems.map((it, idx) => {
    ...
    const price = normalizeMoneyFromApi(rawPrice);
    const total = normalizeMoneyFromApi(rawTotal);
...
export default function CheckoutClient() {
  ...
  // Calcula frete sempre que o CEP mudar
  useEffect(() => {
    if (!selectedCep) {
